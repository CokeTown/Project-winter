...
- **일부 인터페이스 래스터**(인게임 단말/PDA 하우징 · 일지 표지 · 종이 텍스처): **AI 생성 후 픽셀화·후처리**(로고·아이콘 파이프라인으로 도트화 — 게임 픽셀 미감 통일 + 포토리얼 'AI-룩' 제거). ★픽셀화해도 원본이 AI 생성이라 **AI 파생 = 고지 유지**(포토리얼 인상만 제거된 것). 원본 백업 보존.
