...
## 5. AI 콘텐츠 공개 (필수 설문): 정직 고지 — 2026-07-24 정정

- **Pre-Generated AI content: YES / Live-Generated: NO** (런타임 생성 0 — 전부 빌드에 고정·사람 검수)
- **판정 기준**: "AI 결과물이 유저에게 **직접 전달**되는가"(유저가 보고/듣는 AI 산출물만 필수 고지). 코드·개발도구는 유저 비노출이나 정직을 위해 함께 밝힌다.

**유저 직접 노출 AI (필수 고지)**
- **음악(OST)**: AI 음악 도구 **Suno**로 생성. ← 유저가 듣는다. ★이전 문구의 "OST=AI 아님"은 **오류였고 정정**(아래).
- **일부 인터페이스 래스터**(인게임 단말/PDA 하우징 · 종이·지도 텍스처): **AI 생성 후 픽셀화·후처리**(로고·아이콘 파이프라인으로 도트화 — 게임 픽셀 미감 통일 + 포토리얼 'AI-룩' 제거). ★픽셀화해도 원본이 AI 생성이라 **AI 파생 = 고지 유지**(포토리얼 인상만 제거된 것). 원본 백업 보존.
  - *미사용 AI 래스터(일지 표지·tip_scrap·fieldnote·구 로고)는 빌드에서 삭제 — 참조 0(2026-07-25).*
- **현지화(번역)**: 기계 번역 초벌 + 전 문장 사람 검수·수정.

**개발 과정 AI (유저 비노출 — 참고 고지)**
- 게임 코드 일부: 개발자 직접 감독 아래 AI 보조 도구.

**AI 아님 (명시)**: 3D 전부 = 손코딩 절차적 지오메트리(AI 생성 모델 아님) · **UI 아이콘 전부 = 개발 도구 절차 생성**(글리프 84종=`icon-semiotic.mjs` 벡터 SVG · 플랫 PNG 아이콘=`gen-icons.mjs`·`icon-mono-gen.mjs`·`icon-catset.mjs` 등 개발자 도구, **AI 아님** — 디렉터 확인 2026-07-24) · 텍스처 = 절차적 canvas · SFX = 샘플(PSE·녹음) · 내러티브·이벤트 카드·상점 캡슐·스크린샷 = 실제 인게임 렌더/사람 저작.

**EN (설문 서술란)**: "Nine Winters is a solo project; all design, direction, and review are done by a human, with AI used as a tool. Pre-generated AI content in the build: the background music was generated with an AI music tool (Suno); some interface art (the in-game device/PDA housing, journal cover, and paper textures) is AI-generated and is being replaced with authored/procedural art before launch; and machine translation was used as a first-draft aid for localization, with all text reviewed and edited by a human. AI-assisted tools also helped write portions of the game code under direct supervision. The game generates no content with AI at runtime. All 3D is hand-coded procedural geometry (not AI-generated models), all UI icons are procedurally generated by the developer's own tooling, and all store capsules and screenshots are actual in-game renders."

**KO (설문 서술란)**: "1인 개발작으로, 기획·연출·검수는 전부 사람이 하고 AI는 도구로 씁니다. 빌드에 포함된 사전 생성 AI: 배경 음악은 AI 음악 도구(Suno)로 만들었고, 일부 인터페이스 아트(인게임 단말/PDA 하우징·일지 표지·종이 텍스처)는 AI 생성이며 출시 전 저작/절차적 아트로 교체합니다. 현지화는 기계 번역을 초벌로 쓴 뒤 모든 문장을 사람이 검수·수정했습니다. 게임 코드 일부도 직접 감독 아래 AI 보조 도구로 작성했습니다. 게임 내 실시간 AI 생성은 없습니다. 3D는 전부 손으로 짠 절차적 지오메트리(AI 생성 모델 아님), UI 아이콘은 전부 개발자 자체 도구로 절차 생성, 상점 캡슐·스크린샷은 실제 인게임 렌더입니다."

> **정정(2026-07-24)**: 구 문구가 "OST·SFX·복셀·캡슐=AI 아님"이라 적었으나 **OST는 Suno AI 생성 — 오류였고 정정**. 유저가 듣는 AI 산출물이라 필수 고지(미고지 시 환수 리스크 — 커뮤니티 경고 지점). 래스터(PDA·일지·종이)도 AI 생성 확인 → 고지 추가.
> **처리 완료(2026-07-25)**: 인터페이스 래스터(PDA·일지·종이) **픽셀화·후처리 완료** — 포토리얼 'AI-룩' 제거(레딧 "UI looks AI-Made" 실체 해소). **단, 원본이 AI라 AI 파생은 유지** → 고지 존치(픽셀화는 인상 개선이지 AI-성 제거가 아님). ②음악(Suno)은 유지·고지(전면 손번호 비현실적). → 실질 방어선 = **"AI-룩 0(전부 도트로 통일) · 고지는 음악·아트(픽셀화)·번역 정직"**. ★어차피 Suno가 AI 설문 '예'를 강제하므로 아트 고지 존치가 추가 부담 아님.
> **⚠️ 업로드 게이트**: 이 문구는 라이브 스팀 페이지 AI 설문에 반영돼야 유효. 데모 업로드 전 반드시 반영 확인.
> (구 이력 — 2026-07-11: 캡슐→인게임 렌더 교체. 2026-07-12: 번역 항목 추가.)
...
- **AI 생성 콘텐츠 고지(설문 내 별도 섹션)**: §5 정정 문구 사용(2026-07-24) — "1인 개발, 기획·디자인·검수는 사람, AI는 도구. 유저 노출 AI = **음악(Suno)·번역 초벌**(+출시 전 제거 예정인 일부 인터페이스 래스터), 개발용 AI = 코드 보조. 3D·아이콘·텍스처·SFX·캡슐은 AI 아님." EULA와 무관, 이 필드에서 처리(A5). ★음악(Suno) 필수 포함 — 누락 시 미고지.
