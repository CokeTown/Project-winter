...
| **2. 장르** | **기지 건설 · 아이들러 · 생존 · 탐험 · 관리** | 클리커·샌드박스는 스킵(샌드박스는 정식판 무한/배경화면 있으나 코어 아님). |
| **3. 하위 장르** | **생활 시뮬레이션** | 도시 건설(×단일 셸터)·개척 시뮬(×솔로)·생존 공포(×코지) 아님. |
...
| **자유검색(기타)** | **Cozy** (필수) · Wholesome(있으면) | 목차 미수록 → 검색 추가. "Cozy" 안 뜨면 "아늑한" 시도. |
| **9. 평가(최종)** | 검토 후 제출 | — |
