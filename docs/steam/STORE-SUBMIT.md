...
| **3. 하위 장르** | (단계가 보여주는 후보 중 맞는 것 / 없으면 스킵) | 생존 공포(×코지) 아님. 대부분 「기타」로 넘어감. |
| **4. 비주얼 및 관점** | **복셀 · 아이소메트릭 · 3D · 양식화된** | ⚠️ 픽셀 그래픽·2.5D 절대 아님. |
| **5. 테마 및 분위기** | **포스트아포칼립스 · 분위기 있는 · 고양이 · 눈** | 다크·다크 판타지 아님(코지). "눈"이 겨울 태그. |
| **6. 기능** | **기지 건설 · 크래프팅 · 자원관리 · 선택의 중요성 · 복수 결말 · 풍부한 스토리 · 건설** | 엔딩 6종·초대장 선택이 「선택의 중요성」·「복수 결말」 근거. |
| **7. 플레이어** | **싱글플레이어** | 멀티 없음. |
...
| **자유검색(기타)** | **Cozy(필수) · 아이들러 · 생존 · 탐험 · 관리** (+ Wholesome 있으면) | 마법사 단계에 안 뜬 실존 태그는 전부 여기서 검색 추가. "Cozy" 안 뜨면 "아늑한" 시도. |
| **9. 평가(최종)** | 검토 후 제출 | — |

> ⚠️ **마법사 단계 ≠ 목차 카테고리**: 각 단계는 큐레이팅된 부분집합만 보여준다(예: 「장르」 단계에 생존·아이들러 없음).
> 원칙 — **그 단계에 뜨는 것 중 맞는 것만 고르고, 목표 태그가 안 뜨면 「기타」 자유검색으로 넣는다.**
> 최종 20태그(아래)가 다 들어가기만 하면 어느 단계에서 넣었는지는 무관.
...
**핵심**: ①상위 장르 = 캐주얼+시뮬레이션 ②시각 = 복셀/아이소메트릭(픽셀·2.5D 오류 정정) ③생존은 8번째(리드 아님) ④Cozy는 「기타」 자유검색 필수 ⑤등급 태그·자금 태그 = 없음.
