...
|---|---|---|---|---|
| **Nine Winters** | Base App | **$11.49** | 본편 전체(아홉 겨울 · 5모드 · 무료 확장 1.1~1.4 수령) | AppID |
...
| 번들 | 구성 | 개별 합 | 번들가 | 할인 |
|---|---|---|---|---|
| **Supporter Edition** | Base + Supporter Pack | $15.48 | **$13.99** | ~10% |
| **Complete Warmth** | Base + Supporter + OST | $20.47 | **$17.99** | ~12% |
...
## 1. Base — Nine Winters ($11.49)

### 가격 근거
- **완성도 우선 포지셔닝**(v1.5 기준): 콘텐츠 볼륨(셸터 다수 · 아홉 겨울 · 5모드 · 위젯 모드) + **출시 후 무료 확장 4회(1.1~1.4) 로드맵**이 체감 가치를 끌어올린다. 무료 확장 약속은 "$11.49가 지금 사도 계속 커진다"는 신호 → 초기 구매·리뷰 유도.
- 코지/방치 장르 신작 밴드($9.99~14.99)의 하단이 아니라 **중앙**을 택함: 너무 싸면 "가벼운 앱"으로 읽히고, 코지 장르는 가격이 품질 신호로 작동.
- **$8.99 대안**: 신규 개발자의 위시리스트→구매 전환을 극대화하는 심리가($8.99는 강력한 앵커). 단 무료 확장 로드맵의 가치를 스스로 깎는다. → **론치 주간 -10% 할인($10.34)** 으로 $8.99 근처의 전환 심리를 얻으면서 정가 앵커는 $11.49로 유지하는 편이 낫다.
- **결정 포인트(디렉터)**: 정가 $11.49 + 론치 -10% 권장. $8.99로 내릴지는 디렉터 최종 판단(포지셔닝 vs 전환).

### Steam 설정
- Primary genre: **Simulation**; 서브: Indie, Casual.
- 태그(등록순, [PAGE-COPY §3](PAGE-COPY.md)): Idle, Cozy, Survival, Base Building, Post-apocalyptic, Pixel Graphics, Relaxing, Crafting, Exploration, Singleplayer, Atmospheric, Cats, Building, Resource Management, 2.5D.
...
DLC 판매를 위한 최소 연동. 현재 Steam 라이브러리 미탑재 → 그라운드워크부터.

- [ ] **Steam 라이브러리 도입**: `steamworks.js`(Electron/Node FFI) 또는 greenworks. Electron 데스크톱 빌드에만 로드(웹/모바일은 스텁).
...
- [ ] **Steam 언어 연동(#34)**: `steamAPI.utils.getSteamUILanguage()` → 초기 lang 힌트(이미 ko/en i18n 외부화 완료 → 연결만).
...
- 구성: **확정 초안**(디렉터 승인 대기 항목: ① Base 정가 $11.49 vs $8.99, ② 미소유 서포터 항목 "조용히 없음" vs "잠금 노출").
