# Nine Winters: 스팀 페이지 카피 (ko/en)

> 기준: GD-THESIS 한 문장 "세상이 끝나도, 삶은 계속 손질할 수 있다" / 태그라인(디렉터 확정 2026-07-11, 영어 메인) **"Embers in dust"** · 한국어 짝 **"먼지 속 불씨"**. 구 태그라인("폐허에도 온기는 남아 있다")은 폐기.
> 카피 리트머스(THESIS §6) 자체검증은 문서 하단. 지시조 금지·keep-all·병치 공식(회색 세계 × 앰버 온기).

---

## 1. 짧은 설명 (Short Description, 300자 이내)

### KO
<p style="word-break:keep-all">
세상은 이미 끝났고, 되돌릴 수 없다. 그래도 난롯불은 지필 수 있고, 고양이는 곁에 눕는다. <b>Nine Winters</b>는 폐허 속 셸터 하나를 손질하며 아홉 번의 겨울을 나는 코지 생존 게임이다. 창밖으로 잿빛 도시가 얼어붙는 동안, 안에서는 수프를 데우고 벽지를 고쳐 붙이고 라디오를 켠다. 회복은 사건이 아니라 습관이다. 살아남는 게 아니라, 살아보는 이야기.
</p>

### EN
The world already ended, yet the stove still lights and the cat curls up beside you. **Nine Winters** is a cozy survival game about tending one shelter through nine winters. As the grey city freezes outside, you warm the soup, mend a wall, turn on the radio. Recovery is a habit, not an event. Not surviving. Dwelling.

---

## 2. 긴 설명 (About This Game)

### KO

<p style="word-break:keep-all">
눈은 그치지 않는다. 도시는 잿빛으로 얼어붙었고, 거리에는 아무도 남지 않았다. 그 폐허 한가운데, 당신의 셸터에는 아직 불이 켜져 있다.
</p>

<p style="word-break:keep-all">
<b>Nine Winters</b>는 구원을 팔지 않는다. 세계는 이미 끝났으니까. 대신 회복의 가장 작은 단위를 건넨다. 데운 수프 한 캔, 고쳐 단 슬레이트 한 장, 난로 앞에 몸을 만 고양이, 지지직거리는 라디오 한 채널. 매일의 손질이 쌓여 온기가 되고, 그 온기만이 이 폐허에서 유일하게 머무는 것이 된다.
</p>

<p style="word-break:keep-all">
밀수꾼도, 먼 불빛도, 문 앞에 꾸러미를 두고 간 누군가도 흐른다. 머무는 건 당신이 손수 지핀 것뿐이다. 실패해도 죽지 않는다. 그저 조금 덜 온전한 하루가 지날 뿐이다. 재촉하지 않는 세계에서, 아홉 번의 겨울을 당신의 속도로 지나가면 된다.
</p>

<b>이 폐허에서 할 수 있는 것</b>

- <b>아홉 번의 겨울.</b> 계절이 흐르고, 버틴 시간이 세어진다. 여름의 부패, 겨울의 봉쇄, 한파에 서리는 성에까지, 준비의 긴장과 해소가 계절마다 되돌아온다.
- <b>손질과 꾸미기.</b> 벽지와 바닥과 테마로 셸터를 나만의 집으로 바꾼다. 비어 있던 컨테이너가 하나둘 채워지고, 그 변화가 수첩에 "그 해 겨울"로 남는다.
- <b>난로 앞의 고양이.</b> 쓰다듬으면 눈을 감는다. 위안은 고양이가, 일은 개가 맡는다. 동물만이 흐르지 않고 곁에 머문다.
- <b>네 개의 지평선.</b> 얼어붙은 항구, 발밑의 지하 노선, 오로라 뜨는 고원, 그리고 금지 구역. 정식 출시 이후 네 번의 무료 확장으로 셸터에서 닿는 세계가 넓어진다.
- <b>다섯 가지 방식.</b> 느긋한 노말, 시험 같은 하드, 구제 없는 하드코어, 끝도 재촉도 없는 무한, 그리고 압박을 다 겪은 이에게 열리는 배경화면 모드. 저마다 다른 방식으로 온기를 묻는다.
- <b>바탕화면 위젯.</b> 꾸민 집을 바탕화면에 켜둔다. 눈이 내리고 난롯불이 흔들리는 동안, 당신의 온기가 데스크톱 한구석에 머문다.

<p style="word-break:keep-all">
사망 연출은 없다. 몰아세우는 손도 없다. 여기 있는 건 초대뿐이다. 세상이 끝난 뒤에도 삶은 계속 손질할 수 있다는, 그 초대.
</p>

### EN

Snow does not stop. The city has frozen grey, and no one is left on the streets. In the middle of that ruin, your shelter still has its lights on.

**Nine Winters** does not sell salvation. The world already ended. It offers the smallest unit of recovery instead: a warmed can of soup, a re-hung slate, a cat curled by the stove, one crackling channel on the radio. The day's small mending accumulates into warmth, and that warmth becomes the only thing in these ruins that stays.

The smuggler drifts on. So does the far light, and the stranger who left a parcel at your door. What stays is only what you kindle yourself. Failing does not kill you; a slightly less whole day simply passes. In a world that never rushes you, you cross nine winters at your own pace.

**What the ruins hold**

- **Nine winters.** Seasons drift, and the time you endure gets counted. Summer's rot, winter's lockdown, the frost creeping across the glass. The tension of preparing and the relief of surviving return with each season.
- **Tending and decorating.** Wallpaper, flooring, and themes make the shelter your own home. An empty container fills in, piece by piece, and the change is kept in your notebook as "that winter."
- **A cat by the stove.** Pet it, and it closes its eyes. The cat is comfort, the dog is work. Animals are the one thing that stays rather than drifts.
- **Four horizons.** A frozen harbor, the lines beneath your feet, a highland where auroras rise, and the forbidden zone. Four free expansions after launch widen the world your shelter can reach.
- **Five ways to dwell.** Easygoing Normal, testing Hard, unforgiving Hardcore, the endless Endless, and the Wallpaper mode that opens only to those who have weathered the pressure. Each asks about warmth in a different way.
- **A desktop widget.** Leave your finished home glowing on the desktop. While the snow falls and the firelight flickers, your warmth stays in one corner of the screen.

There is no death scene. No hand pushing you forward. What is here is only an invitation: that even after the world ends, life is something you can keep tending.

---

## 3. 태그 (스팀 등록 순 20개: 가중치는 앞쪽이 크다, 실제 목차 검증 2026-07-11)

아늑함, 기지 건설, 생활 시뮬레이션, 아이들러, 고양이, 장식, 포스트아포칼립스, 릴랙싱, 생존, 크래프팅, 자원관리, 탐험, 분위기 있는, 눈, 건전함, 풍부한 스토리, 복수 결말, 감성적, 복셀, 싱글플레이어

> **아늑함(Cozy)·건전함(Wholesome)은 마법사 「테마 및 분위기」 단계에 실존**, 자유검색 불요(2026-07-11 실물 확인).
> 여유분/러너업(20 초과 시): 선택의 중요성, 깊은 세계관, 인디, 아이소메트릭, 웅장한 사운드트랙, 컨트롤러, 미스터리.
> 정정(2026-07-11): 구 목록의 `Pixel Graphics`·`2.5D`는 오류, 게임 렌더는 **3D 복셀 + 아이소메트릭**(픽셀아트는 인카운터 삽화뿐). `복셀`로 교체.
> 순서: COMMS-KIT §1("살림을 판다, 서바이벌 아님"), Cozy/기지 건설/생활 시뮬레이션 앞세우고 생존은 8번째.

---

## 4. 데모 설명문 ("첫 번째 겨울")

### KO
<p style="word-break:keep-all">
<b>첫 번째 겨울.</b> 데모에서는 셸터 하나에 불을 지피고, 눈이 도시를 덮는 48일을 지나본다. 수프를 데우고, 벽을 고치고, 고양이를 곁에 두는 하루의 결이 여기 다 있다.<br>
Day 48, 첫 겨울의 끝에서 화면이 잠시 멈춘다. <b>여덟 번의 겨울이 남아 있다.</b>
</p>

### EN
**The first winter.** The demo lights one shelter and carries you through the 48 days as snow buries the city. The texture of a day, warming the soup, mending a wall, keeping the cat close, is all here.
At Day 48, the end of the first winter, the screen holds still for a moment. **Eight winters remain.**

---

## 5. Next Fest 소개문 (단문)

### KO
<p style="word-break:keep-all">
눈은 그치지 않고, 도시는 잿빛으로 얼었다. 그래도 난롯불은 지필 수 있다. 폐허 속 셸터를 손질하며 아홉 번의 겨울을 나는 코지 생존, 그 첫 번째 겨울을 지금 데모로 만나보자.
</p>

### EN
Snow does not stop, and the city has frozen grey. Even so, the stove still lights. A cozy survival about tending a shelter through nine winters in the ruins. Play the first winter now in the demo.

---

## 6. 카피 리트머스 자체검증 (THESIS §6)

| 질문 | 판정 | 근거 |
|---|---|---|
| ① 온기를 **만들·쌓·돌아보게** 하는가? | 통과 | 짧은 설명("회복은 습관"), 긴 설명 불릿 전부가 손질→축적→회고(수첩 "그 해 겨울") 축으로 정렬. |
| ② 몰아세우는가, **초대**하는가? | 통과 | 지시조("~하라") 0건. "지나가면 된다 / 초대뿐이다"로 마감. 명령형 대신 서술·청유. |
| ③ 타인이 **머무는가?** | 통과(캐논 준수) | 밀수꾼·먼 불빛·문 앞의 누군가 모두 "흐른다"로 명시. 머무는 건 "손수 지핀 온기"와 동물(정착의 예외)뿐. |

추가 점검: 병치 공식(회색 세계 × 앰버 온기), 모든 문단이 "잿빛 도시/폐허" + "난롯불/수프/고양이/불빛"을 한 문장 안에 병치. keep-all 조판 적용(word-break:keep-all). 이모지는 모드명 배지(게임 내 문법)에서만 예외 허용, 본문 카피에는 미사용.
