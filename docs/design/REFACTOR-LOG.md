...
- **F3 (✅ 시스템 그라운딩 QA, 2026-07-07)**: 오프스크린 하네스가 **실접지 QA를 가능케 함**(구 `QA-REPORT.md` v0.9.1의 "프리뷰차단" 한계 극복 — 이제 렌더/로밍/모달/예외를 실런타임에서 실측). 이번 세션 산출물 전수 점검 **13/13 green + 버그 0**: ①지식 트리 모달 렌더(4갈래×티어1/2/3, 선행 게이팅, 익힘/배우기/선행필요 3상태, 해금 파이프라인 책 차감·중복차단) ②가방 확정 파밍(밴드 diff-0, 그물) ③게이트 코스트 모드 스케일(단조성, 그물) ④야생동물 로밍(강제스폰 3개체·1.9s Δ25.6 이동·야간발자국 0→7·groundY=-0.75 셸터지면 정확 접지) ⑤sim 60일×4모드 무예외 ⑥전 페이즈 런타임 예외 0건. *스크립트: `scratchpad/qa-systems.cjs` (harness 재사용, 페이즈별 try/catch 격리).*
