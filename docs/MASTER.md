...
### 2026-07-22 (수) · 항공뷰 지도 — 설계 2판 + S1 라이브 프로토
- **트랙 진척도**: **항공 지도(신규 트랙) 0% → S1 프로토 완료**(설계 2판 확정 + 라이브 디오라마 실구현 + 판단물 제출) · UI 리워크: HUD 2차 0% 유지 · 리팩터: P1 부분(조명 인자화)이 지도 S2의 전제로 승격.
- **한 일**: ①`AERIAL-MAP.md` 설계→**개정 1차**(디렉터 확정: 실시간·데모 필수·프로토로 판단 → 베이크안 폐기, **단일 라이브 씬+카메라 2상태** 아키텍처) ②`map-aerial` 브랜치에 S1 프로토 실구현 — 인스턴싱 도시 680블록·시간대 조명 키프레임(밤=달)·눈 3면 스왑·overview↔focus 돌리 줌·**본편 픽셀화 파이프 공유**(renderFrame 분기, 시뮬 틱 유지=실시간 공짜) ③튜닝 4라운드 후 8컷 매트릭스 아티팩트 제출.
- **산출**: 커밋 `9b84ae0`·`33c1fee`(설계) `8266337`(S1) · 브랜치 `map-aerial` 푸시 · 판단물 아티팩트(시간대×날씨 8컷).
- **검증**: 배터리 102/102 · **골든 17/17(0.114%) — 프로토 추가 본편 무회귀** · 하드코드 0.
- **다음 한 수**: 디렉터 S1 판정〔D〕→ GO 시 S2(노드 DOM 오버레이·정보 카드·mapview 추출·출발 연결·조명 함수 본편 통일=REFACTOR P1 부분). 8월 중 판정이 데모 유입(S3)의 안전선.
- **⚠️ 함정 기록**: 팩토리 반환에 메서드 미노출 → 매 프레임 TypeError → renderFrame 사망 → **캔버스 정지 프레임이 정상 렌더로 위장**(DOM 독립 합성). 오프스크린 검증 시 에러 리스너 선부착 필수.
