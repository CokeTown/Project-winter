...
- **🔥 자율 진행 큐 (디렉터 2026-07-23 확정 — 순서대로, 각 항목 완료 후 자동으로 다음)**:
  ...
  3. **게임 로비(타이틀) 개편** — 좌: 메뉴 현행 유지(이어하기·새 게임 등, **언어 설정만 제거**) + **종료 버튼 신설**. 우: 지금처럼 살짝 윗각에서 셸터를 내려다보는 라이브 뷰, **상호작용 가능 유지**. 레퍼런스 영상(X 링크)은 로그인 월로 미열람 — 구도 설명이 스펙 정본, 필요 시 디렉터에게 캡처 요청.
