...
### 잔여 (2단계 후보)
- 탐험 포함 소크(simDays 기본은 정착 경제만 — 모드별 부상·pity·hardLoot 경로는 배터리 golden이 normal/hard 커버, 혹한/무한 탐험 소크 미실측)
- 자원 고갈 시나리오: wallpaper·zen에서 food/water 0 도달 시 무력 상태 처리(꾸미기=무사망 설계 확인)
- 모드 전환 왕복(세이브 로드 시 모드 보존)
