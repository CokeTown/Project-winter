...
function readLocaleOverrides() {
  ...
  for (const d of dirs) {
    const ko = load(d, 'ko.json'), en = load(d, 'en.json'), ja = load(d, 'ja.json');
    if (ko || en || ja) return { ko, en, ja, dir: d };
  }
  return { ko: null, en: null, ja: null, dir: null };
}
let overrides = { ko: null, en: null, ja: null, dir: null };
