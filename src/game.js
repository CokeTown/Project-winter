...
import { PAINT_FAMILIES, paintFamilyOf } from './data/paints.js'; // 도료 12계열 (REWARD-LOOP ② — 데모 포팅 #149. MEMOS_CITYCORE는 2.0 전용이라 미포함)
...
function updateWindowSkies() {
  ...
  // #161 계절 팔레트(1.7.3 데모 포팅): 창밖 하늘도 계절을 따른다 — 봄 청록기/여름 맑은 하늘색/가을 노란기/겨울 창백.
  //   눈/폭풍이면 캘린더 무관 겨울 하늘 — 데모 첫눈 climax가 여름 캘린더(day13~)여도 차갑게(「보름의 겨울」 정합).
...
function applyTimeLighting() {
  ...
  // #161 계절 팔레트(1.7.3 데모 포팅): 계절이 대기의 색으로 읽히게 — 지역 틴트(#54)와 같은 문법, 그보다 옅게.
  //   봄=풋내 나는 초록기 / 여름=쨍한 백금색 / 가을=금빛 마른 잎 / 겨울=차가운 청회색. 낮에만(dayness 가중), 급변 없음(lerp).
  //   눈/폭풍이면 캘린더 무관 겨울 틴트 — 첫눈/한파는 여름 캘린더여도 겨울 정서(데모 「보름의 겨울」 정합).
...
function recordNormalDay(day) {
  if (_simRunning || state.qaUsed || DEMO_ED) return; // 시뮬·QA·데모는 계정 통계 오염 금지 (#90: 공유 nw-stats에 데모 기록 누출 차단)
  const s = readStats();
  let dirty = false;
...
function doSaveNow() {
  ...
  state.layouts[state.current] = items.map(i => ({ d: i.defId, c: i.colorIdx, x: +i.x.toFixed(3), z: +i.z.toFixed(3), r: i.rot, o: i.on === false ? 0 : 1, y: +(i.y || 0).toFixed(2), t: i.tier || 0 }));
...
function buildItemGroup(item) {
  ...
  const g = def.build(def.colors[item.colorIdx], item.colorIdx, null, item.tier || 3); // 4인자: 가구 티어(#157) — 데모는 스케치(3인자) 미사용
  item.glowMeshes = [];
  g.traverse(o => {
    if (o.isMesh) {
...
function loadShelter(id) {
  ...
  for (const it of (state.layouts[id] || [])) {
    ...
    addItem(it.d, it.c ?? 0, cx, cz, it.r ?? 0, it.o !== 0, it.y || 0, it.t ?? 3);
  }
  for (const it of items) {
    if (!it.y) continue;
...
function openMapModal() {
  ...
  for (const [rid, r] of Object.entries(REGIONS)) {
    ...
    //   (본선의 숙련 별★ 표식은 데모 미포함 — 지역 숙련은 2.0 기능)
  ...
  $('modal-body').querySelectorAll('button[data-rproj]').forEach(b =>
    b.addEventListener('click', () => {
      if (b.dataset.rproj === 'slate') {
        ...
        state.layouts.rooftop = items.map(i => ({ d: i.defId, c: i.colorIdx, x: +i.x.toFixed(3), z: +i.z.toFixed(3), r: i.rot, o: i.on === false ? 0 : 1, t: i.tier || 0 }));
        loadShelter('rooftop');
        closeModal();
  ...
  $('modal-body').querySelectorAll('button[data-mod]').forEach(b =>
    b.addEventListener('click', () => {
      ...
      if (id === 'extension') {
        ...
        state.layouts[state.current] = items.map(i => ({ d: i.defId, c: i.colorIdx, x: +i.x.toFixed(3), z: +i.z.toFixed(3), r: i.rot, o: i.on === false ? 0 : 1, t: i.tier || 0 }));
...
function rebuildBunkerGeometry() {
  ...
  state.layouts.bunker = items.map(i => ({ d: i.defId, c: i.colorIdx, x: +i.x.toFixed(3), z: +i.z.toFixed(3), r: i.rot, o: i.on === false ? 0 : 1, t: i.tier || 0 }));
  loadShelter('bunker');
  closeModal();
  shadowDirty();
}
...
function rebuildShelterGeometry() {
  ...
  state.layouts[id] = items.map(i => ({ d: i.defId, c: i.colorIdx, x: +i.x.toFixed(3), z: +i.z.toFixed(3), r: i.rot, o: i.on === false ? 0 : 1, t: i.tier || 0 }));
  loadShelter(id);
  closeModal();
  shadowDirty();
}
...
function processDay() {
  ...
  if (!isWallpaper()) {
    ...
    // 데모 티저(디렉터 승인): Day 6 슬럼 전당포 1회 고정 스폰 — "이런 게 떠오르는구나"를 보름 안에 반드시 한 번.
    if (DEMO_ED && state.day === 6 && !state.fieldSpot && !state.demoSpotSeeded) {
      state.demoSpotSeeded = true;
      state.fieldSpot = { id: 'pawnshop', region: 'slum', until: state.day + BAL.fieldSpots.lifeDays };
      notes.push(t('spot.appeared', { name: t('spot.pawnshop'), region: LName(REGIONS.slum) }));
    }
    // 스폰: 스팟 없을 때만, 해금 지역 한정.
    if (!state.fieldSpot && dayRand(1) < BAL.fieldSpots.spawnChance) {
      ...
      if (cands.length) {
        const ws = cands.map(() => 1); // 데모: 지역 숙련 미포함 — 균등 가중 (본선은 숙련 티어 가중)
...
function openQaPanel() {
  ...
  $('modal-body').querySelectorAll('[data-qa]').forEach(b => b.addEventListener('click', () => {
    ...
    switch (k) {
      ...
      // 도료 검수용 — 전 계열 +3 (스와치 게이트·소모·도감 확인) — 데모 포팅 #149 (2.0 히든 점프는 미포함)
...
window.__shelter = {
  ...
  PAINT_FAMILIES, paintFamilyOf, rollPaintFamily, jackpotToast, showSelPanel, DEFS, BAL, // 도료 (REWARD-LOOP ② — 데모 포팅 #149) + 데이터 핸들
  rollOfflineGift, // DDD-5 복귀 서프라이즈 (데모 포팅 #149, 코어 테스트용)
