...
export const STR = {
  ...
  'map.riskLine':     { ko: '위험 {risk} · 이동 포함 <b>{dur}초</b> (거리 ×{mult}) · {wicon} {wname}{forecast}', en: 'Risk {risk} · incl. travel <b>{dur}s</b> (distance ×{mult}) · {wicon} {wname}{forecast}' },
  ...
  'prep.riskLine':    { ko: '위험도: {risk} · 소요 {dur}초{sprain}{mobile} · {wicon} {wname}{forecast}', en: 'Risk: {risk} · takes {dur}s{sprain}{mobile} · {wicon} {wname}{forecast}' },
  ...
  'prep.depart':      { ko: '🎒 출발 ({dur}초)', en: '🎒 Depart ({dur}s)' },
  ...
  'exp.timeLeft':     { ko: '{n}초 남음', en: '{n}s left' },
  ...
  'injury.card':      { ko: '<span class="i-name">{icon} {name}</span> — 탐험 성공률 -{pen}%p{time}<br>자연 회복까지 약 <span id="injury-eta">{h}</span>시간 (휴식)', en: '<span class="i-name">{icon} {name}</span> — expedition success -{pen}%p{time}<br>~<span id="injury-eta">{h}</span>h until natural recovery (rest)' },
